...
class Product
{
    ...
    public function update($id, $name, $price, $quantity, $description, $category_id, $image_urls = [], $main_image_index = null)
    {
        try {
            if ($this->conn === null) {
                throw new Exception("Không thể kết nối cơ sở dữ liệu.");
            ...
            if (!empty($image_urls)) {
                // Reset existing main image flag
                $stmt = $this->conn->prepare("
                ...
                // Insert new images
                foreach ($image_urls as $index => $image_url) {
                    $is_main = ($index == $main_image_index) ? true : false;
                    $stmt = $this->conn->prepare("
                        INSERT INTO product_images (product_id, image_url, is_main, created_at)
                        VALUES (:product_id, :image_url, :is_main, NOW())
                    ");
            ...
            } elseif ($main_image_index !== null) {
                // Update main image if no new images uploaded but main image index changed
                $stmt = $this->conn->prepare("
                    UPDATE product_images 
                    SET is_main = FALSE 
                    WHERE product_id = :id
                ...
                $stmt = $this->conn->prepare("
                    ...
                    SET is_main = TRUE 
                    WHERE product_id = :id 
                    AND image_url = (SELECT image_url FROM product_images WHERE product_id = :id LIMIT 1 OFFSET :offset)
                ");
                $stmt->bindParam(':id', $id, PDO::PARAM_INT);
                $stmt->bindParam(':offset', $main_image_index, PDO::PARAM_INT);
                $stmt->execute();
            }
    ...
    public function searchByName($keyword, $limit, $offset, $sort = 'ASC', $category_id = null)
    {
        try {
            ...
            error_log("searchByName SQL: $sql");
            error_log("searchByName params: " . json_encode($params));
            ...
            error_log("searchByName rows: " . count($rows));
            ...
            error_log("searchByName products: " . count($products));
            return $products;
        } catch (Exception $e) {
    ...
    public function getPaginated($limit, $offset, $sort = 'ASC')
    {
        try {
            ...
            $stmt = $this->conn->prepare("
                ...
                FROM products p
                LEFT JOIN product_images pi ON p.ID = pi.product_id
                ORDER BY p.price $sort, pi.is_main DESC, pi.created_at ASC
                LIMIT :limit OFFSET :offset
            ");
            $stmt->bindParam(':limit', $limit, PDO::PARAM_INT);
            $stmt->bindParam(':offset', $offset, PDO::PARAM_INT);
